...
Sk³adowa si³y grawitacji prostopad³a do powierzchni stoku czyli nacisk na powierzchniê stoku wynosi:\\
\begin{equation}
\label{eq:Fn}
F_n = mg\cos\alpha
\end{equation}
...
W wielu jêzykach dostêpne s± funkcje biblioteczne, które znajduj± rozwi±zywania równañ ró¿niczkowych zwyczajnych pierwszego rzêdu, takich jak nasz uk³ad \ref{eq:ur2d}. W naszym przypadku wykorzysta³y¶my funkcjê dopri (ang. \textit{Numerical integration of ODE using Dormand-Prince RK method}) z biblioteki Numeric Javascript.
...
Za³ó¿my, ¿e narciarz porusza siê tylko po liniach prostych. W dalszej czê¶ci pracy poka¿emy, ¿e takie ograniczenie mo¿e byæ dobrym sposobem na przybli¿enie rzeczywistego toru jazdy, a znacznie u³atwia sterowanie ruchem narciarza. \\
...
Podstawiaj±c te informacje do uk³adu równañ \ref{eq:ur2rzedu}, otrzymujemy:
...
W celu jak najdok³adniejszego zamodelowania zmian prêdko¶ci podczas poruszania siê po ³amanej, testowanych by³o szereg modeli. Konieczno¶æ wprowadzenia takich mechanizmów staje siê oczywista, gdy u¶wiadomimy sobie, ¿e im wiêkszy jest k±t pomiêdzy kolejnymi odcinkami pokonywanej ³amanej, tym bardziej w realnej sytuacji, kosztowne pod wzglêdem wytraty prêdko¶ci jest to krok. Dla zobrazowania sytuacji, rozwa¿my k±t 90 stopni. Aby przej¶æ ze stanu jechania w linii spadku stoku do jechania w skos stoku, narciarz musi przyhamowaæ do zera. Z kolei im k±t jest bardziej zbli¿ony do 180 stopni, nie ma efektu przyhamowania, b±d¼ przyhamowanie jest minimalne, gdy¿ w realnym przypadku, zmiana kierunku jazdy jest wykonywana w p³ynnym ³uku.
...
Pierwsza strategia opiera siê na obliczanie kolejno drugich pochodnych funkcji w ka¿dym kolejnym punkcie trasy a nastêpnie zsumowaniu ich bezwzglêdnych warto¶ci. 

\[ \ddot{d_i} = \frac{x_1 + x_3 - 2x_2}{(y_3 -y_2) (y_2-y_1) } \]   

\[ k =  \sum\limits_{i=1}^{n-1} |\ddot{d_i}|^{3}  \]   

Powsta³± liczbê $k$ mo¿na traktowaæ jako wspó³czynnik, którego ma³a bezwzglêdna warto¶æ oznacza bardzo ³agodne k±ty przej¶cia na ca³ej trasie natomiast im wiêksz± kanciasto¶æ trasy tym implikuje ona wiêksz± warto¶æ tego wspó³czynnika. Aby wzmocniæ efektywno¶æ wspó³czynnika, dodatkowo, przed zsumowaniem, warto¶æ bezwzglêdna drugiej pochodnej jest podnoszona do trzeciej potêgi. Wyliczony wspó³czynnik dla ka¿dej z rozwa¿anych w algorytmie tras jest mno¿ony przez obliczony czas przejazdu tej trasy. Dopiero taka liczba jest traktowana jako funkcji celu algorytmu genetycznego. Minusem tej strategi jest kompletne zniekszta³cenie wyników tj. realnego czasu przejazdu narciarza po zadanym torze.
...
Ostatnia strategia polega na zliczaniu ilo¶ci zmian kierunku jazdy na testowanym torze i porównanie jej z ilo¶ci± bramek wymuszaj±cych skrêt na trasie. Otrzymujemy zatem ilo¶æ nadprogramowych zmian krawêdzi, które w ka¿dym przypadku powoduj± gorszy czas przejazdu ni¿ gdyby ich nie by³o. Aby to zatem zamodelowaæ, w tej strategi dodajemy do wyliczonego czasu przejazdu sta³± ilo¶ci sekund kary za ka¿d± nadprogramow± zmianê krawêdzi i to traktujemy jako warto¶æ funkcji celu algorytmu genetycznego.
